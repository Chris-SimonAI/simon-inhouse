...
export interface BarVisualizerProps
  ...
  centerAlign?: boolean
  /** Custom bar color class (e.g., 'bg-white') */
  barColor?: string
}
...
const Bar = React.memo<{
  ...
  state?: AgentState
  barColor?: string
}>(({ heightPct, isHighlighted, state, barColor }) => (
  <div
    data-highlighted={isHighlighted}
    className={cn(
      "max-w-[12px] min-w-[8px] flex-1 transition-all duration-150",
      "rounded-full",
      barColor || "bg-border data-[highlighted=true]:bg-primary",
      !barColor && state === "speaking" && "bg-primary",
